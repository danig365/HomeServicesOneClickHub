...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Image, Alert, Modal, Linking, TextInput } from 'react-native';
...
export default function MyHomeScreen() {
  ...
  const { properties, getSelectedProperty, selectProperty, addInsight, updateInsight, deleteInsight, getUpcomingReminders, getOverdueReminders } = useProperties();
  const { currentUser } = useUser();
  const { getSubscription, createSubscription, getNextVisit, getRecentVisits } = useSubscription();
  ...
  const recentVisits = property && subscription ? getRecentVisits(property.id, 3) : [];
  
  const [showPropertySelector, setShowPropertySelector] = useState(false);
  const [showRescheduleModal, setShowRescheduleModal] = useState(false);
  const [selectedService, setSelectedService] = useState<any>(null);
  const [showInsightModal, setShowInsightModal] = useState(false);
  const [editingInsight, setEditingInsight] = useState<PropertyInsight | null>(null);
    ...
    return (
      <View style={styles.container}>
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>Please select a property to view your smart home</Text>
        </View>
      </View>
    );
  }
  ...
  const getLastMaintenanceDate = (taskName: string) => {
    ...
    for (const visit of allVisits.slice().reverse()) {
      if (visit.status === 'completed') {
        const task = visit.tasks.find(t => t.name.toLowerCase().includes(taskName.toLowerCase()));
  ...
  const propertyRecurringServices = recurringServices.filter(s => 
    s.propertyId === property.id && s.status === 'active'
  );
  ...
  const maintenanceInsights = [...defaultInsights, ...customInsights.map(insight => ({
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.blueprintSection}>
          {subscription.blueprint ? (
            ...
            >
              <View style={styles.blueprintHeader}>
                ...
                <View style={styles.blueprintInfo}>
                  ...
                  <Text style={styles.blueprintSubtitle}>Your 5-Year Home Plan</Text>
                </View>
              ...
              <View style={styles.blueprintStats}>
                ...
                <View style={styles.blueprintStat}>
                  ...
                  <Text style={styles.blueprintStatText}>
                    {subscription.blueprint.customProjects?.length || 0} Projects
                  </Text>
                </View>
                <View style={styles.blueprintStat}>
                  <Icons.ListChecks size={16} color={COLORS.teal} />
                  <Text style={styles.blueprintStatText}>
                    {subscription.blueprint.monthlyVisitRequests?.length || 0}/5 Requests
                  </Text>
                </View>
              </View>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={styles.blueprintEmptyTitle}>Create Your Blueprint</Text>
              <Text style={styles.blueprintEmptyText}>
                Start your 5-year home improvement plan to help us personalize your maintenance
              </Text>
        ...
        <View style={styles.scoreSection}>
          <View style={styles.scoreSectionHeader}>
            ...
            {subscription.currentScore && (
              <Text style={styles.scoreQuarter}>
                {subscription.currentScore.quarter} {subscription.currentScore.year}
              </Text>
            )}
          </View>
          ...
          {subscription.currentScore ? (
            <View style={styles.scoreCard}>
              <View style={styles.scoreMainWrapper}>
                <View style={styles.scoreCircle}>
                  <Text style={styles.scoreMainValue}>{subscription.currentScore.score}</Text>
                  <Text style={styles.scoreMainLabel}>Overall</Text>
                </View>
                <View style={styles.scoreDescription}>
                  <Text style={styles.scoreDescriptionTitle}>Excellent Condition</Text>
              ...
              <View style={styles.categoriesGrid}>
                {Object.entries(subscription.currentScore.categories).map(([key, value]) => {
                  ...
                  const categoryColor = value >= 90 ? '#10B981' : value >= 80 ? '#F59E0B' : '#EF4444';
                  
                  return (
                    <View key={key} style={styles.categoryItem}>
                      <View style={[styles.categoryIconWrapper, { backgroundColor: `${categoryColor}15` }]}>
                        <Icon size={18} color={categoryColor} />
                      </View>
                      <Text style={styles.categoryLabel}>
                        {key.charAt(0).toUpperCase() + key.slice(1)}
                      </Text>
                      <Text style={[styles.categoryValue, { color: categoryColor }]}>{value}</Text>
                    </View>
                  );
                })}
              </View>
            ...
            <View style={styles.scoreEmptyCard}>
              ...
              <Text style={styles.scoreEmptyText}>Score will be available after your first visit</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.insightsGrid}>
            {maintenanceInsights.map((insight, index) => {
              ...
              const isOverdue = daysSince !== null && daysSince > insight.recommended;
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => {
                    if (isCustom && property) {
                      const customInsight = property.insights?.find(i => i.id === (insight as any).id);
                      if (customInsight) {
                        setEditingInsight(customInsight);
                        setInsightForm({
                          label: customInsight.label,
                          lastUpdated: customInsight.lastUpdated,
                          icon: customInsight.icon,
                          color: customInsight.color,
                          recommendedInterval: customInsight.recommendedInterval || 90,
                  ...
                  disabled={!isCustom}
                >
                  <View style={[styles.insightIconWrapper, { backgroundColor: `${insight.color}20` }]}>
                    <Icon size={20} color={insight.color} />
                  </View>
                  <Text style={styles.insightLabel}>{insight.label}</Text>
                  <Text style={styles.insightDate}>{insight.lastCleaned}</Text>
                  {daysSince !== null && (
                    <View style={[styles.insightBadge, isOverdue && styles.insightBadgeOverdue]}>
                      <Text style={[styles.insightBadgeText, isOverdue && styles.insightBadgeTextOverdue]}>
                        {daysSince} days ago
                      </Text>
                    </View>
                  )}
            ...
            <View style={styles.nextVisitCard}>
              ...
              <View style={styles.nextVisitHudson}>
                {subscription.personalDirector?.photo ? (
                  ...
                  <View style={[styles.hudsonPhoto, styles.hudsonPhotoPlaceholder]}>
                ...
                <View style={styles.hudsonInfo}>
                  ...
                  <Text style={styles.hudsonRole}>Your Personal Home Director</Text>
                </View>
              </View>
                ...
                <View style={styles.recurringHeader}>
                  <View style={styles.recurringLeft}>
                    ...
                    <View>
                      ...
                      <Text style={styles.recurringFrequency}>{service.frequency}</Text>
                    </View>
                  </View>
                  <TouchableOpacity 
            ...
            {recentVisits.map((visit) => (
              <View key={visit.id} style={styles.recentVisitCard}>
                <View style={styles.recentVisitHeader}>
                  ...
                  <Text style={styles.recentVisitDate}>{formatDate(visit.completedDate!)}</Text>
                </View>
        ...
        {property && (getUpcomingReminders(property.id).length > 0 || getOverdueReminders(property.id).length > 0) && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
            ...
            {getUpcomingReminders(property.id).slice(0, 2).map((reminder: PropertyReminder) => (
              <View key={reminder.id} style={styles.reminderPreviewCard}>
                <View style={styles.reminderPreviewHeader}>
                  ...
                  <View style={styles.propertyOptionLeft}>
                    ...
                    <View>
                      <Text style={[
                        ...
                        prop.id === property.id && styles.propertyOptionNameSelected
                      ]}>
                        {prop.name}
                      </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.rescheduleContent}>
              <Text style={styles.rescheduleServiceName}>{selectedService?.serviceName}</Text>
              <Text style={styles.rescheduleCurrentDate}>
                Current: {selectedService && formatDate(selectedService.nextServiceDate)}
              </Text>
                      ...
                      { text: 'Cancel', style: 'cancel' },
                      {
                        ...
                        onPress: () => {
                          if (subscription?.personalDirector.phone) {
                            Linking.openURL(`tel:${subscription.personalDirector.phone}`);
                          }
                        },
                      },
                    ]
            ...
            <ScrollView style={styles.modalScroll}>
              <View style={styles.formSection}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setInsightForm({ ...insightForm, label: text })}
                />
              </View>
              ...
              <View style={styles.formSection}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setInsightForm({ ...insightForm, lastUpdated: text + 'T00:00:00.000Z' })}
                />
              </View>
              ...
              <View style={styles.formSection}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.iconSelector}>
                  {['Wind', 'Droplet', 'Zap', 'Home', 'Wrench', 'Hammer', 'Paintbrush', 'Thermometer', 'Lightbulb', 'Flame', 'Snowflake', 'Fan', 'Gauge'].map((iconName) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.iconOption,
                          insightForm.icon === iconName && styles.iconOptionSelected
                        ]}
                        onPress={() => setInsightForm({ ...insightForm, icon: iconName })}
                      >
                        <IconComponent size={24} color={insightForm.icon === iconName ? COLORS.teal : '#6B7280'} />
                      </TouchableOpacity>
                    );
                  })}
                </ScrollView>
              ...
              <View style={styles.formSection}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.colorSelector}>
                  {['#60A5FA', '#3B82F6', '#FCD34D', '#10B981', '#EF4444', '#8B5CF6', '#EC4899', '#F97316'].map((color) => (
                    <TouchableOpacity
                      key={color}
                      style={[
                        styles.colorOption,
                        { backgroundColor: color },
                        insightForm.color === color && styles.colorOptionSelected
                      ]}
              ...
              <View style={styles.formSection}>
                <Text style={styles.formLabel}>Recommended Interval (days)</Text>
                <TextInput
                  style={styles.formInput}
                  placeholder="90"
                  keyboardType="numeric"
                  value={insightForm.recommendedInterval?.toString()}
                  onChangeText={(text) => setInsightForm({ ...insightForm, recommendedInterval: parseInt(text) || 90 })}
                />
              </View>
              ...
              <View style={styles.formSection}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setInsightForm({ ...insightForm, notes: text })}
                          ...
                          { text: 'Cancel', style: 'cancel' },
                          {
                            ...
                            onPress: async () => {
                              if (property && editingInsight) {
                                await deleteInsight(property.id, editingInsight.id);
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    const insightData = {
                      ...
                      updatedByRole: currentUser?.role || 'homeowner',
                    };

                    if (editingInsight) {
                      await updateInsight(property.id, editingInsight.id, insightData);
